...
func TestCreation(t *testing.T) {
	...
	forwarder, err := newTcpGraphiteCarbonForwarer("0.0.0.0", 12345, time.Second, 10, "", 1)
	assert.Equal(t, nil, err, "Expect no error")
	...
	assert.Equal(t, "bob.metric", dp.Metric(), "Expect metric back")
...
func TestDeadlineError(t *testing.T) {
	...
	l, err := listener.CarbonListenerLoader(forwardTo, &listenFrom)
	defer l.Close()
	carbonForwarder, err := newTcpGraphiteCarbonForwarer("0.0.0.0", 12246, time.Second, 10, "", 1)
	assert.Equal(t, nil, err, "Expect no error")
...
func TestWriteError(t *testing.T) {
	...
	l, err := listener.CarbonListenerLoader(forwardTo, &listenFrom)
	defer l.Close()
	forwarder, err := newTcpGraphiteCarbonForwarer("0.0.0.0", 12347, time.Second, 10, "", 1)
	assert.Equal(t, nil, err, "Expect no error")
...
func TestCarbonWrite(t *testing.T) {
	...
	assert.Equal(t, nil, err, "Expect no error")
	assert.Equal(t, 4, len(l.GetStats()), "Expect no stats")
	forwarder, err := newTcpGraphiteCarbonForwarer("0.0.0.0", 12348, time.Second, 10, "", 1)
	assert.Equal(t, nil, err, "Expect no error")
	assert.Equal(t, "", forwarder.Name(), "Expect no name")
	assert.Equal(t, 0, len(forwarder.GetStats()), "Expect no stats")
	forwarder.openConnection = nil // Connection should remake itself
...
func TestFailedConn(t *testing.T) {
	...
	assert.Equal(t, nil, err, "Expect no error")
	assert.Equal(t, 4, len(l.GetStats()), "Expect no stats")
	forwarder, err := newTcpGraphiteCarbonForwarer("0.0.0.0", 12349, time.Second, 10, "", 1)
	assert.Equal(t, nil, err, "Expect no error")
	assert.Equal(t, "", forwarder.Name(), "Expect no name")
	assert.Equal(t, 0, len(forwarder.GetStats()), "Expect no stats")
	forwarder.openConnection = nil // Connection should remake itself
