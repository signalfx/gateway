...
#
godep go test -cover -covermode atomic -race -parallel=8 -timeout 3s -cpu 4  ./... | grep -av 'skiptestcoverage' | grep -av "100.0% of statements" > /tmp/no_100_coverage
...
# ---- Run benchmarks only
#
godep go test -run=none -bench=. -benchtime 10ms ./... || exit 1
echo "OK!"
