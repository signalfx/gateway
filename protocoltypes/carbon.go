...
func NewCarbonDatapoint(line string, metricDeconstructor metricdeconstructor.MetricDeconstructor) (CarbonReadyDatapoint, error) {
	...
	if err == nil {
		return &carbonDatapointImpl{
			core.NewAbsoluteTimeDatapoint(
				metricName, dimensions, value.NewIntWire(metricValueInt),
	...
	return &carbonDatapointImpl{
		core.NewAbsoluteTimeDatapoint(
			metricName, dimensions, value.NewFloatWire(metricValueFloat),
			com_signalfuse_metrics_protobuf.MetricType_GAUGE,
			time.Unix(0, metricTime*int64(time.Second))),
		line,
	}, nil
