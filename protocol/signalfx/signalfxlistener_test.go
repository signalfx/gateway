...
func TestSignalfxListener(t *testing.T) {
	Convey("given a signalfx listener", t, func() {
		...
		Convey("invalid requests should error", func() {
			...
			verifyStatusCode("INVALID_JSON", "", "/v1/datapoint", http.StatusBadRequest)
			verifyStatusCode("INVALID_JSON", "", "/v2/datapoint", http.StatusBadRequest)
			verifyStatusCode("INVALID_JSON", "", "/datapoint", http.StatusBadRequest)
			verifyStatusCode("INVALID_JSON", "", "/v2/event", http.StatusBadRequest)
			verifyStatusCode("INVALID_JSON", "", "/v1/trace", http.StatusBadRequest)
		...
		Convey("Should be able to send a v1 JSON point to /datapoint", func() {
			datapointSent := datapoint.Datapoint{
				Metric:     "bob",
				Value:      datapoint.NewIntValue(3),
				Dimensions: map[string]string{"sf_source": "name"},
			}
			trySend(`{"metric":"bob", "source": "name", "value": 3}`, "application/json", "/datapoint")
			datapointSeen := sendTo.Next()
			datapointSent.Timestamp = datapointSeen.Timestamp
			So(datapointSent.String(), ShouldEqual, datapointSeen.String())
		})
		...
		Convey("Should be able to send a v1 protobuf point on /datapoint", func() {
			datapointSent := datapoint.Datapoint{
				Metric: "metric",
				Value:  datapoint.NewIntValue(3),
			}
			...
			_, err = body.Write(dpBytes)
			So(err, ShouldBeNil)
			trySend(body.String(), "application/x-protobuf", "/datapoint")
			datapointSeen := sendTo.Next()
			datapointSent.Timestamp = datapointSeen.Timestamp
			So(datapointSent.String(), ShouldEqual, datapointSeen.String())
			So(len(datapointSeen.Meta), ShouldEqual, 0)
